...
BSP_STATIC_ASSERT(MRFI_ADDR_SIZE ==
                  ((sizeof(mrfiBroadcastAddr) /
                    sizeof(mrfiBroadcastAddr[0])) * sizeof(mrfiBroadcastAddr[0])));
...
/* Packet automation control - base value is power up value whick has APPEND_STATUS enabled; no CRC
 *autoflush */
...
const uint8_t mrfiRadioCfg[][2] =
{
    ...
    {  IOCFG0,    MRFI_SETTING_IOCFG0       }, /* Configure GDO_0 to output PA_PD signal (low during
                                                *TX, high otherwise). */
    {  IOCFG1,    MRFI_SETTING_IOCFG1       }, /* Configure GDO_1 to output RSSI_VALID signal (high
    ...
    /* [BM] For the ISM_US configuration FREQ2, FREQ1 and FREQ0
     *  are set later on depending upon Chronos version
     *  (Black or White PCB)
     */
#ifndef ISM_US
    /* [BM] ISM_EU and ISM_LF */
...
//};
/* verify number of table entries matches the corresponding #define */
BSP_STATIC_ASSERT(__mrfi_NUM_LOGICAL_CHANS__ ==
                  ((sizeof(mrfiLogicalChanTable) /
                    sizeof(mrfiLogicalChanTable[0])) * sizeof(mrfiLogicalChanTable[0])));
...
BSP_STATIC_ASSERT(__mrfi_NUM_POWER_SETTINGS__ ==
                  ((sizeof(mrfiRFPowerTable) /
                    sizeof(mrfiRFPowerTable[0])) * sizeof(mrfiRFPowerTable[0])));
...
void MRFI_Init(void)
{
    ...
    /* initialize radio registers */
    {
       ...
       {  // [BM] Chronos with Black PCB
    	   ...
    	   MRFI_RADIO_REG_WRITE(FREQ1, SMARTRF_SETTING_FREQ1_B);
    	   MRFI_RADIO_REG_WRITE(FREQ0, SMARTRF_SETTING_FREQ0_B);
       }
       else
       { // [BM] Chronos with White PCB
    	   MRFI_RADIO_REG_WRITE(FREQ2, SMARTRF_SETTING_FREQ2_W);
     ...
     */
    {
       ...
       {  // [BM] Chronos with Black PCB
    	   ...
    	   MRFI_ASSERT(SMARTRF_SETTING_FREQ1_B == MRFI_RADIO_REG_READ(FREQ1));
    	   MRFI_ASSERT(SMARTRF_SETTING_FREQ0_B == MRFI_RADIO_REG_READ(FREQ0));
       }
       else
       { // [BM] Chronos with White PCB
    	   MRFI_ASSERT(SMARTRF_SETTING_FREQ2_W == MRFI_RADIO_REG_READ(FREQ2));
                ...
                {
                    {
                        ...
                        mrfiIncomingPacket.rxMetrics[MRFI_RX_METRICS_RSSI_OFS] =
                            Mrfi_CalculateRssi(mrfiIncomingPacket.rxMetrics[
 ...
 **************************************************************************************************
 */
// [BM] Changed because CC1101_VECTOR ISR is declared of source code and this handler is called
// indirect
...
void MRFI_RadioIsr(void)
{
    ...
    if (interfaceIntSource)
    {
        ...
        {
            /* Not expecting any other interface interrupts (data in/out, status/instr in, fifo
             *rx/tx) */
            MRFI_FORCE_ASSERT();
        }
    }
...
{
    MRFI_ASSERT(mrfiRxFilterAddr[0] != mrfiBroadcastAddr[0]); /* filter address must be set before
                                                               *enabling filter */
...
#define MRFI_RADIO_MAX_PKTLEN       (MRFI_RADIO_TX_FIFO_SIZE - MRFI_LENGTH_FIELD_SIZE - \
...
#    error \
    "ERROR:  Maximum possible value for PKTLEN exceeded.  Decrease value of maximum payload."
#endif
...
#    error \
    "ERROR:  Maximum possible packet length exceeds FIFO buffer.  Decrease value of maximum payload."
#endif
 ...
 */
BSP_STATIC_ASSERT(sizeof(mrfiRadioCfg) ==
                  ((sizeof(mrfiRadioCfg) / sizeof(mrfiRadioCfg[0])) * sizeof(mrfiRadioCfg[0])));
 ...
 *  comment out the offending asserts.
 */
BSP_STATIC_ASSERT(sizeof(mrfiLogicalChanTable) ==
                  ((sizeof(mrfiLogicalChanTable) /
                    sizeof(mrfiLogicalChanTable[0])) * sizeof(mrfiLogicalChanTable[0])));
BSP_STATIC_ASSERT(sizeof(mrfiBroadcastAddr) ==
                  ((sizeof(mrfiBroadcastAddr) /
                    sizeof(mrfiBroadcastAddr[0])) * sizeof(mrfiBroadcastAddr[0])));
